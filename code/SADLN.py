...
class SADLN():
    def __init__(self, datasets, n_latent_dim,weight=0.001, model_path='SADLN.h5', epochs=600, batch_size=64, learning_rate=0.0003):
        self.latent_dim = n_latent_dim
        # optimizer = Adam()
        # TODO 将keras自带的优化器替换为tf的优化器，解决随机性问题
        optimizer =tf.train.AdamOptimizer(learning_rate=0.0003)  # 调参， 更换优化器 Adam(), AdadeltaOptimizer(),
        self.n = len(datasets)
        self.epochs = epochs
    ...
    def build_encoder(self):
        ...
        if self.n > 1:
            merged_dense = concatenate(denses, axis=-1) #将四种数据拼接
        else:
...
class SADLN_API(object):
    def __init__(self, model_path='./model/', epochs=600, weight=0.001,batch_size=64, learning_rate=0.0003):
        self.model_path = model_path
    ...
    # feature extract
    def feature_gan(self, datasets, index=None, n_components=100, b_decomposition=True, bTrain= True, weight=0.0001):
        if b_decomposition:
    ...
    def encoder_gan(self, ldata, n_components=100,bTrain=True):
...
def main(argv=sys.argv):
    ...
    # TODO 调参 epochs 迭代次数 200， 400， 600， 1000， 2000， 5000， 10000
    ...
    SADLN = SADLN_API(model_path, epochs=args.epochs, weight=args.disc_weight, batch_size=64, learning_rate=0.0003)
    ...
    if args.run_mode == 'SADLN':
        ...
        # 维度为(属性个数，100)，将其保存到fea/cancer_type.fea文件下，在第二个命令中将被用到 # TODO 重要
        # n_components用于控制encoder输出的latent code的维度
        vec = SADLN.feature_gan(ldata, index=l, n_components=100,
                                     bTrain= True, # TODO bTrain, 是否重新训练or加载之前的模型预测
...
###############################################################################################################
        ...
        if isfile(fea_tmp_file):
            ...
            if isfile(label_file):
                ...
                plt.legend(['c1', 'c2','c3','c4','c5'],title="Cluster",loc=1, bbox_to_anchor=(1.105,1.0),borderaxespad = 0.)
                plt.show()
        # cancer_type = args.type
